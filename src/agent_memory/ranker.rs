...
impl Ranker {
    ...
    fn type_bonus(&self, intent: QueryIntent, memory_type: MemoryType) -> f32 {
    ...
    fn recency_bonus(&self, intent: QueryIntent, timestamp: DateTime<Utc>, now: DateTime<Utc>) -> f32 {
        let age_days = (now.timestamp() - timestamp.timestamp()).max(0) as f32 / 86_400.0;
        match intent {
            QueryIntent::TaskState | QueryIntent::EpisodicRecall => (1.0 - (age_days / 30.0)).clamp(0.0, 1.0),
